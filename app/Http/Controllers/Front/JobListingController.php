...
class JobListingController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        if ($job_single !== null) {
            $jobs = Job::with('rCompany', 'rJobCategory', 'rJobLocation', 'rJobType', 'rJobExperience', 'rJobGender', 'rJobSalaryRange')
                ->where('job_category_id', $job_single->job_category_id)
                ->get();
        } else {
            // Xử lý khi $job_single là null
            // Ví dụ: Gán giá trị mặc định hoặc hiển thị thông báo lỗi
        }
